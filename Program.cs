...
foreach (string name in studentNames)
{
    ...
    double assignmentExtraScore = 0;

    double currentStudentGrade = 0;

    int gradedAssignments = 0;

    double overallStudentGrade = 0;
    ...
    foreach (double score in studentScores)
    {
        ...
        {
            assignmentExtraScore += score / 10;
        } 
    }

    currentStudentGrade = sumAssignmentScores / examAssignments;

    overallStudentGrade = (sumAssignmentScores + assignmentExtraScore) / examAssignments;
    ...
    Console.WriteLine($"{currentStudent}\t\t{currentStudentGrade}\t\t{overallStudentGrade:0.00}\t{currentStudentLetterGrade}\t{(int)currentStudentGrade} ({assignmentExtraScore/5:0.00} pts)");
}
